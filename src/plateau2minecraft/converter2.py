...
class Minecraft:
    # Minecraftクラスは、マージされたポイントクラウドデータをMinecraftのリージョンファイルに変換する役割を担います。
    ...
    # Minecraftリージョンを構築するメソッド
    # このメソッドはポイントクラウドデータからMinecraftのリージョンファイルを作成し、指定された出力ディレクトリに保存します。
    # 入力: 出力ディレクトリ (Path), オプションでワールドの原点座標 (tuple)
    # 出力: 指定された出力ディレクトリに保存されたリージョンファイル
